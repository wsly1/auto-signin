...
Windows 下可以不依赖 Codex，通过启动文件夹快捷方式在登录后自动签到。脚本会等待网络可用，然后优先运行同目录下的 `auto-signin-cli.exe`；如果没有 exe，则回退到系统 `python signin_tool.py -c accounts.json sign`。

运行时会在系统托盘显示图标，右键菜单可以点 `Exit` 退出；签到完成后会弹出托盘提醒，并在约 8 秒后自动退出。日志会保存到 `logs` 文件夹。
...
如果电脑平时不关机，可以额外安装每天定时签到任务。默认每天 `06:00` 和 `15:00` 各尝试一次，仍然使用同一个托盘脚本，完成后自动退出：

```powershell
powershell -ExecutionPolicy Bypass -File .\install_daily_tasks.ps1
```

卸载每日定时任务：

```powershell
powershell -ExecutionPolicy Bypass -File .\uninstall_daily_tasks.ps1
```
...
日常签到时，如果森空岛返回 `401/code=10000` 这类 `cred_token` 过期错误，工具会自动使用已保存的 `cred` 刷新 `cred_token`，保存到 `accounts.json`，然后重试本次签到。正常情况下每天只需要打开工具点“一键签到”。
